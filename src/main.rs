...
impl SimpleServer {
  ...
  fn simple_db_call (&mut self, number: u32) -> impl Future<Item = Response<services::NumberMessage>, Error = tower_grpc::Error> {
    ...
    let query = connection.and_then(move |conn| conn.prep_exec("SELECT :number AS TEST", params!{ number} ));
...
impl services::server::SimpleService for SimpleServer {
  ...
  type GetNumberFuture = Box<futures::Future<Item = tower_grpc::Response<services::NumberMessage>, Error = tower_grpc::Error>>;
...
fn main() {
  ...
  let MYSQL_URL = env::var("MYSQL_URL").unwrap_or_else(|_| {
    println!("Required environment variable MYSQL_URL not found, exiting");
    process::exit(1);
  });
  ...
  let new_service = services::server::SimpleServiceServer::new(SimpleServer::new(MYSQL_URL, &reactor));
